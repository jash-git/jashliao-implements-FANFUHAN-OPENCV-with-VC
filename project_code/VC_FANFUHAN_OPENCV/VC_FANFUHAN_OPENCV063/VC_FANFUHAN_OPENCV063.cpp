// VC_FANFUHAN_OPENCV063.cpp : 定義主控台應用程式的進入點。
//
/*
// Debug | x32
通用屬性
...
int main()
{
	...
	else
	{
		...
		// 膨胀
		/*
		膨脹的API -在窗中，只要含有１，則窗內全變為１ [白色/mask]；可以增加淺色成分 淺色成分得膨脹
		void cv::dilate(InputArray src,OutputArray dst,InputArray kernel,Point anchor = Point(-1,-1),int iterations = 1,int borderType = BORDER_CONSTANT,const Scalar & borderValue = morphologyDefaultBorderValue())
		參數:
		src 輸入圖像，任意通道的
		dst 輸出圖像，類型與通道數目必須跟輸入保持一致
		kernel 結構元素
		anchor 中心位置錨定
		iterations 循環次數
		borderType 邊緣填充類型
		*/
		...
		// 腐蚀
		/*
		腐蝕的API -在窗中，只要含有０，則窗內全變為０ [黑色]；可以去淺色噪點 淺色成分被腐蝕
		...
		*/
		...
		// 霍夫圆检测
		int dp = 4; // 在其它参数保持不变的情况下。dp的取值越高，越容易检测到圆
		int min_radius = 20;
		int max_radius = 100;
		int minDist = 10;
		...
		參數:
		image表示輸入單通道的灰度圖像
		circles 表示檢測的圓信息（圓心+半徑）
		method 圓檢測的方法
		dp表示圖像分辨率是否有變化，默認1表示保持跟原圖大小一致，一般取2，比1 要大
		minDist表示檢測到的圓，兩個圓心之間的最小距離
		param1 表示邊緣提取的高閾值
		param2表示霍夫空間的累加閾值
		minRadius 表示可以檢測圓的最小半徑
		maxRadius 表示可以檢測圓的最大
		*/
		HoughCircles(eresult, circles, HOUGH_GRADIENT, dp, minDist,
		...
		imshow("eresult-result_dp=4", src);
		waitKey(0);
		return 0;

// VC_FANFUHAN_OPENCV024.cpp : 定義主控台應用程式的進入點。
//
/*
// Debug | x32
通用屬性
...
int main()
{
	...
	else
	{
		...
		Mat src_clone01 = src.clone();
		Mat src_clone02 = src.clone();

		// 产生椒盐噪声
		add_salt_pepper_noise(src_clone01);
		imshow("input_src(pepper_noise)", src_clone01);
		// 产生高斯噪声
		add_gaussian_noise(src_clone02);
		imshow("input_src(gaussian_noise)", src_clone02);

		Mat dst1, dst2;
		medianBlur(src_clone01, dst1, 5);//中值濾波/中值模糊(medianBlur)
		imshow("medianBlur ksize=5(pepper_noise)", dst1);

		GaussianBlur(src_clone01, dst2, Size(5, 5), 15, 0);
		imshow("gaussian ksize=5(pepper_noise)", dst2);//高斯模糊(GaussianBlur)

		Mat dst3, dst4;
		medianBlur(src_clone02, dst3, 5);//中值濾波/中值模糊(medianBlur)
		imshow("medianBlur ksize=5(gaussian_noise)", dst3);

		GaussianBlur(src_clone02, dst4, Size(5, 5), 15, 0);
		imshow("gaussian ksize=5(gaussian_noise)", dst4);//高斯模糊(GaussianBlur)
		waitKey(0);
	}

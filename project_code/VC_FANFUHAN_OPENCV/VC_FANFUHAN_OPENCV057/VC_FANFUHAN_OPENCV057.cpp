// VC_FANFUHAN_OPENCV057.cpp : 定義主控台應用程式的進入點。
//
/*
// Debug | x32
通用屬性
...
int main()
{
	Mat src = imread("../../images/my_mask.png");//Mat src = imread("../../images/test.png");
	if (src.empty())
	{
	...
	else
	{
		...
		Mat image1 = Mat::zeros(src.size(), CV_32FC3);
		// 对轮廓内外的点进行分类
		for (int row = 0; row < src.rows; ++row)
		{
			for (int col = 0; col < src.cols; ++col)
			{
				...
				Contour輪廓所有點的集合
				Pt 圖像中的任意一點
				MeasureDist如果是True，則返回每個點到輪廓的距離，如果是False則返回+1，0，-1三個值，其中+1表示點在輪廓內部，0表示點在輪廓上，-1表示點在輪廓外
				*/
				double dist = pointPolygonTest(contours1[0], Point(col, row), true);
				if (dist == 0) {
					image1.at<Vec3f>(row, col) = Vec3f(255, 255, 255);
				}
				else if (dist > 0) {
					image1.at<Vec3f>(row, col) = Vec3f(255 - dist, 0, 0);
				}
				else {
					image1.at<Vec3f>(row, col) = Vec3f(0, 0, 255 + dist);
				}
			}
		}
		convertScaleAbs(image1, image1);
		imshow("points1", image1);

		Mat image2 = Mat::zeros(src.size(), CV_32FC3);
		// 对轮廓内外的点进行分类
		for (int row = 0; row < src.rows; ++row)
		{
			for (int col = 0; col < src.cols; ++col)
			{
				...
				MeasureDist如果是True，則返回每個點到輪廓的距離，如果是False則返回+1，0，-1三個值，其中+1表示點在輪廓內部，0表示點在輪廓上，-1表示點在輪廓外
				*/
				double dist = pointPolygonTest(contours2[0], Point(col, row), true);
				if (dist == 0) {
					image2.at<Vec3f>(row, col) = Vec3f(255, 255, 255);
				}
				else if (dist > 0) {
					image2.at<Vec3f>(row, col) = Vec3f(255 - dist, 0, 0);
				}
				else {
					image2.at<Vec3f>(row, col) = Vec3f(0, 0, 255 + dist);
				}
			}
		}
		convertScaleAbs(image2, image2);
		imshow("points2", image2);
		waitKey(0);
		return 0;

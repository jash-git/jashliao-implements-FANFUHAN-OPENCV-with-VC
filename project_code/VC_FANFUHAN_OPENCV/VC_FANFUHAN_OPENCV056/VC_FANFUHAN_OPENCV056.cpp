// VC_FANFUHAN_OPENCV056.cpp : 定義主控台應用程式的進入點。
//
/*
// Debug | x32
通用屬性
...
int main()
{
	Mat src = imread("../../images/twolines.png");//Mat src = imread("../../images/test.png");
	if (src.empty())
	{
	...
	else
	{
		...
		for (size_t t = 0; t < contours2.size(); ++t) {
			...
			points表示待擬合的輸入點集合
			line在二維擬合時候輸出的是vec4f類型的數據，在三維擬合的時候輸出是vec6f的vector
			distType表示在擬合時候使用距離計算公式是哪一種，OpenCV支持如下六種方式：
			DIST_L1 = 1
			DIST_L2 = 2
			DIST_L12 = 4
			DIST_FAIR = 5
			DIST_WELSCH = 6
			DIST_HUBER = 7
			param對模型擬合距離計算公式需要參數C，5~7 distType需要參數C
			reps與aeps是指對擬合結果的精度要求，一般取0.01
			*/
			fitLine(contours2[t], oneline, DIST_L1, 0, 0.01, 0.01);
			float dx = oneline[0];
			float dy = oneline[1];
			float x0 = oneline[2];
			float y0 = oneline[3];
			...
			int maxx = 0, maxy = 0;
			for (int i = 0; i < contours2[t].size(); ++i) {
				Point pt = contours2[t][i];
				if (miny > pt.y) {
					miny = pt.y;
				}
				if (maxy < pt.y) {

// VC_FANFUHAN_OPENCV054.cpp : 定義主控台應用程式的進入點。
//
/*
// Debug | x32
通用屬性
	...
	else
	{
		...
		for (size_t t = 0; t < contours1.size(); t++) {
			...
			try
			{
				...
				-角度
				*/
				RotatedRect rrt = fitEllipse(contours1[t]);
		...
		for (size_t t = 0; t < contours2.size(); t++) {
			...
			try
			{
				...
				-角度
				*/
				RotatedRect rrt = fitEllipse(contours2[t]);
				ellipse(src01, rrt, Scalar(0, 0, 255), 2);
			}
			catch (...)
			{

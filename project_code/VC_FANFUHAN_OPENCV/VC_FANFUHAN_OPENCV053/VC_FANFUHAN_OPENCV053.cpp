// VC_FANFUHAN_OPENCV053.cpp : 定義主控台應用程式的進入點。
//
/*
// Debug | x32
通用屬性
...
int main()
{
	Mat src = imread("../../images/abc.png");//Mat src = imread("../../images/test.png");
	...
	else
	{
		...
		contours_info(src, contours1);//contours_info(src, contours1, 0, 255);//
		contours_info(src2, contours2);//contours_info(src2, contours2, 0, 255);//

		// 几何矩计算与hu矩计算
		/*
		OpenCV中可以通過如下的API一次計算出上述三種矩，API如下：
		...
		*/
		...
		for (size_t t = 0; t < contours1.size(); ++t) {
			...
			HuMoments(mm, hu);
			/*
			然后我們可以使用hu矩作為輸入，對輪廓進行匹配，進行輪廓外形匹配的API如下：
			double matchShapes(InputArray contour1,InputArray contour2,int method,double parameter)
			contour1第一個輪廓點集合，或者灰度圖像
			contour2第二個輪廓點集合，或者灰度圖像
			method表示比較方法，最常見有
			CONTOURS_MATCH_I1
			CONTOURS_MATCH_I2
			CONTOURS_MATCH_I3

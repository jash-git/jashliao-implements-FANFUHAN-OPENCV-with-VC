// VC_FANFUHAN_OPENCV081.cpp : 定義主控台應用程式的進入點。
//
/*
// Debug | x32
通用屬性
...
int main()
{
	Mat src = imread("../../images/box.png");//Mat src = imread("../../images/test.png");
	if (src.empty())
	{
	...
	else
	{
		...
		imshow("Harris_normalize_ScaleAbs", dst_norm);
		...
		for (int row = 0; row < dst_norm.rows; ++row) {
			for (int col = 0; col < dst_norm.cols; ++col) {
				...
				if (rsp > 150) {
					...
					circle(src, Point(row, col), 5, Scalar(b, g, r), 2);
				}
			}
		}

// VC_FANFUHAN_OPENCV050.cpp : 定義主控台應用程式的進入點。
//
/*
// Debug | x32
通用屬性
...
int main()
{
	Mat src = imread("../../images/zhifang_ball.png");//Mat src = imread("../../images/test.png");
	if (src.empty())
	{
	...
	else
	{
		...
		*/
		...
		for (int t = 0; t < contours00.size(); ++t) {
			...
			輸入參數points可以一系列點的集合，對輪廓來說就是該輪廓的點集
			返回結果是一個矩形，x, y, w, h
			*/
			Rect rect = boundingRect(contours00[t]);
			rectangle(src, rect, Scalar(0, 0, 255));

			// 面积与弧长过滤
			double area = contourArea(contours00[t]);
			double curvelen = arcLength(contours00[t], true);
		...
		for (int t = 0; t < contours01.size(); ++t) {
			// 最大外接矩形
			Rect rect = boundingRect(contours01[t]);
			rectangle(src01, rect, Scalar(0, 0, 255));
			...
			計算輪廓的面積，其原理是基於格林公式。
			參數contour表示輸入的輪廓點集
			參數oriented默認是false返回的面積是正數，如果方向參數為true表示會根據是順時針或者逆時針方向返回正值或者負值面積。
			*/
			double area = contourArea(contours01[t]);

			/*
			OpenCV對輪廓點集計算弧長的API函數如下:
			double cv::arcLength(InputArray curve,bool closed)
			參數curve表示輸入的輪廓點集
			參數closed默認表示是否閉合區域
			*/
			double curvelen = arcLength(contours01[t], true);
			if (area < 100 || curvelen < 100) {
				continue;
			}
			...
			// 绘制旋转矩形与中心位置
			for (int i = 0; i < 4; ++i) {
				line(src01, pts[i % 4], pts[(i + 1) % 4], Scalar(0, 255, 0), 2);
			}
			Point2f cpt = rrt.center;
			circle(src01, cpt, 2, Scalar(255, 0, 0), 2);
		}

// VC_FANFUHAN_OPENCV055.cpp : 定義主控台應用程式的進入點。
//
/*
// Debug | x32
通用屬性
...
int main()
{
	Mat src = imread("../../images/hand.jpg");//Mat src = imread("../../images/test.png");
	if (src.empty())
	{
	...
	else
	{
		...
		// 因為白色在此圖像為連通元件(mask)，所以使用開運算删除小的白色雜訊，藉此提高連通元件(mask)準確度
		/*
		...
		for (size_t t = 0; t < contours.size(); t++) {
			...
			*/
			...
			int len = hull.size();
			for (int i = 0; i < len; ++i) {
				circle(src, hull[i], 4, Scalar(255, 0, 0), 2);//標示節點
				line(src, hull[i%len], hull[(i + 1) % len], Scalar(0, 0, 255), 2);//繪出連接線
			}
		}

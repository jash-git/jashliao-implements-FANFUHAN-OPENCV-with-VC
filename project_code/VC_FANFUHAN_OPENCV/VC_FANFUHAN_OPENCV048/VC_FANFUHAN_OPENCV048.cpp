// VC_FANFUHAN_OPENCV048.cpp : 定義主控台應用程式的進入點。
//
/*
// Debug | x32
通用屬性
...
int main()
{
	Mat src = imread("../../images/coins.jpg");//Mat src = imread("../../images/test.png");
	if (src.empty())
	{
		...
		*/
		...
		for (int t = 0; t < contours.size(); ++t) {
			...
			各個參數詳解如下：
				...
				contourIdx：是指定某個contours的點集；
				color：被填充的顏色，單色可以設置為Scalar(255)等；
				thickness: 所畫Contour的線條寬度，如果為負或CV_FILLED則繪制所有的Contours；
				lineType： 線的連通性；
				hierarchy：可選層次信息結搆，這里面是findContours所的到的基於Contours的層級信息；
				maxLevel： 繪制輪廓的最大等級。如果等級為0，繪制單獨的輪廓。如果為1，繪制輪廓及在其后的相同的級別下輪廓。如果值為2，所有的輪廓。如果等級為2，繪制所有同級輪廓及所有低一級輪廓，諸此種種。如果值為負數，函數不繪制同級輪廓，但會升序繪制直到級別為abs(max_level)-1的子輪廓
				offset：照給出的偏移量移動每一個輪廓點坐標.當輪廓是從某些感興趣區域(ROI)中提取的然后需要在運算中考慮ROI偏移量時，將會用到這個參數。
			*/
			drawContours(src, contours, t, Scalar(0, 0, 255), 2, 8);//邊框畫線
		}
		imshow("contours", src);
		waitKey(0);
		return 0;

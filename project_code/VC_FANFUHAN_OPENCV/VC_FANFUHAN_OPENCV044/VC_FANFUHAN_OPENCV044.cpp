// VC_FANFUHAN_OPENCV044.cpp : 定義主控台應用程式的進入點。
//
/*
// Debug | x32
通用屬性
...
int main()
{
	...
	else
	{
		...
		for (int i = 0; i < 5; ++i) {
			...
			//        THRESH_TOZERO = 3
			//        THRESH_TOZERO_INV = 4
			threshold(gray, binary, 0, 255, i | THRESH_TRIANGLE);
			imshow(format("binary_%d | THRESH_TRIANGLE", i), binary);
		}
		...
		void adaptiveThreshold(InputArray src,OutputArray dst,double maxValue,int adaptiveMethod,int thresholdType,int blockSize,double C)
			...
			當閾值操作類型thresholdType為：THRESH_BINARY
				二值圖像 = 原圖 – 均值圖像 > -C ? 255 : 0
			當閾值操作類型thresholdType為：THRESH_BINARY_INV
				二值圖像 = 原圖 – 均s值圖像 > -C ? 0 : 255
		*/
		adaptiveThreshold(gray, binary2, 255, ADAPTIVE_THRESH_GAUSSIAN_C,THRESH_BINARY, 25, 10);

// VC_FANFUHAN_OPENCV074.cpp : 定義主控台應用程式的進入點。
//
/*
// Debug | x32
通用屬性
...
int main()
{
	Mat src = imread("../../images/case6.jpg");//Mat src = imread("../../images/test.png");
	if (src.empty())
	{
	...
	else
	{
		...
		for (size_t t = 0; t < contours.size(); ++t) {
			...
			double area = contourArea(contours[t]);//計算八方鍊碼的面積
			if (area > max) {
				max = area;
				index = t;
			}
		...
		Mat pts;
		drawContours(src, contours, index, Scalar(0, 0, 255));//繪製輪廓

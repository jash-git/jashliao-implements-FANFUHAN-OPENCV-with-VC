// VC_FANFUHAN_OPENCV076.cpp : 定義主控台應用程式的進入點。
//
/*
// Debug | x32
通用屬性
...
int main()
{
	Mat src = imread("../../images/case1r.png");//Mat src = imread("../../images/test.png");
	if (src.empty())
	{
	...
	else
	{
		...
		threshold(gray, binary, 0, 255, THRESH_BINARY_INV | THRESH_OTSU);//THRESH_BINARY
		...
		// 绘制关键点
		Mat result01 = Mat::zeros(src.size(), src.type());
		Mat pts;
		drawContours(result01, contours, index, Scalar(0, 0, 255));//繪製輪廓
		approxPolyDP(contours[index], pts, 4, true);//輪廓點進行多邊形擬合 ~ 計算輪廓關鍵點
		for (int i = 0; i < pts.rows; ++i) {
			Vec2i pt = pts.at<Vec2i>(i, 0);
			circle(result01, Point(pt[0], pt[1]), 2, Scalar(0, 255, 0), 2);
		}
		imshow("result_buf", result01);
		...
		*/
		Mat M = findHomography(src_pts, dst_pts);//計算透视变换矩陣
		Mat result = Mat::zeros(Size(width, height), CV_8UC3);

		warpPerspective(src, result, M, result.size());//透视变换

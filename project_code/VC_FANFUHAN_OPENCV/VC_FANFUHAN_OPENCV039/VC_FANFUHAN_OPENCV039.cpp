// VC_FANFUHAN_OPENCV039.cpp : 定義主控台應用程式的進入點。
//
/*
// Debug | x32
通用屬性
...
int main()
{
	Mat src = imread("../../images/llk.jpg");//Mat src = imread("../../images/test.png");
	...
	else
	{
		...
		showHistogram(tpl, "Histogram_match_template");
		...
		/*
		void matchTemplate(cv::InputArrayimage,cv::InputArray templ,cV::OutputArray result,int method);
		該函數第一個參數是源圖像，
		第二個參數是模板圖像，
		第三個參數是匹配的結果圖像
		第四個參數是用於指定比較的方法，opencv中支持的比較方法有六種
		01、cv::TM_SQDIFF：該方法使用平方差進行匹配，因此最佳的匹配結果在結果為0處，值越大匹配結果越差。
		02、cv::TM_SQDIFF_NORMED：該方法使用歸一化的平方差進行匹配，最佳匹配也在結果為0處。
		03、cv::TM_CCORR：相關性匹配方法，該方法使用源圖像與模板圖像的卷積結果進行匹配，因此，最佳匹配位置在值最大處，值越小匹配結果越差。
		04、cv::TM_CCORR_NORMED：歸一化的相關性匹配方法，與相關性匹配方法類似，最佳匹配位置也是在值最大處。
		05、cv::TM_CCOEFF：相關性系數匹配方法，該方法使用源圖像與其均值的差、模板與其均值的差二者之間的相關性進行匹配，最佳匹配結果在值等於1處，最差匹配結果在值等於-1處，值等於0直接表示二者不相關。
		06、cv::TM_CCOEFF_NORMED：歸一化的相關性系數匹配方法，正值表示匹配的結果較好，負值則表示匹配的效果較差，也是值越大，匹配效果也好。
		*/
		matchTemplate(src, tpl, result, TM_CCOEFF_NORMED);
		imshow("result", result);//相似度的亮度圖
			...
			for (int col = 0; col < result.cols; ++col) {
				...
				if (v > t) {
					rectangle(src, Point(col, row),
						Point(col + tpl.cols, row + tpl.rows), Scalar(255, 0, 0));//圈選相似候選區域
				}
			}
		}

// VC_FANFUHAN_OPENCV075.cpp : 定義主控台應用程式的進入點。
//
/*
// Debug | x32
通用屬性
...
int main()
{
	Mat src = imread("../../images/wm.jpg");//Mat src = imread("../../images/test.png");
	if (src.empty())
	{
	...
	else
	{
		...
		inRange(hsv, Scalar(100, 43, 46), Scalar(124, 255, 255), mask);//用HSV過濾藍色
		Mat se = getStructuringElement(MORPH_RECT, Size(5, 5));
		imshow("mask1", mask);
		dilate(mask, mask, se);//膨脹 - 可以增加淺色成分
		imshow("mask2", mask);

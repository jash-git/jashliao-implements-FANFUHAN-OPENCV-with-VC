// VC_FANFUHAN_OPENCV088.cpp : 定義主控台應用程式的進入點。
//
/*
// Debug | x32
通用屬性
...
int main()
{
	...
	VideoCapture capture("../../images/balltest.mp4");
	if (!capture.isOpened()) {
	...
	else
	{
		...
		capture.read(frame);
		Rect selection = selectROI("CamShift Demo", frame, true, false);//滑鼠定義ROI起始座標範圍位置
		...
		while (true) {
			...
			cvtColor(image, hsv, COLOR_BGR2HSV);//轉HSV

			inRange(hsv, Scalar(26, 43, 46), Scalar(34, 255, 255), mask);//黃色HSV遮罩(mask)
			int ch[] = { 0, 0 };
			hue.create(hsv.size(), hsv.depth());
			mixChannels(&hsv, 1, &hue, 1, ch, 1);//mixChannels()函数用于将输入数组的指定通道复制到输出数组的指定通道
			...
			// Perform meanShift
			calcBackProject(&hue, 1, 0, hist, backproj, &phranges);//反向投影原理
			backproj &= mask;
			meanShift(backproj, trackWindow, TermCriteria(TermCriteria::EPS | TermCriteria::COUNT, 10, 1));//均值遷移

// VC_FANFUHAN_OPENCV121.cpp : 定義主控台應用程式的進入點。
//
/*
// Debug | x32
通用屬性
...
//文章: https://mp.weixin.qq.com/s/GC1Az0BSkBb8KaUdU3Aj6A
...
int main()
{
	Mat src = imread("../../images/yuanyuan.jpg");//Mat src = imread("../../images/test.png");
	if (src.empty())
	{
	...
	else
	{
		...
		*/
		pencilSketch(src, imgGray, imgColor, 5, 0.1f, 0.03f);//灰色铅笔画、彩色铅笔画效果
		...
		sigma_r	取值范围0~1
		*/
		stylization(src, result, 5, 0.6);//卡通画效果
		imshow("Stylization", result);

		//OilPaint_forloop
		//https://github.com/housewing/OpenCV-Image-Processing/blob/master/OilPaint_forloop.cpp
		Mat OilPaint = Mat(src.size(), CV_8UC3);
		int heigh = src.rows;
		int width = src.cols;
		int BrushSize = 5;
		int levels = 20;
		...
		for (int y = BrushSize; y < (heigh - BrushSize); y++)
		{
			for (int x = BrushSize; x < (width - BrushSize); x++)
			{
				...
				OilPaint.at<Vec3b>(y, x) = pixelColor;

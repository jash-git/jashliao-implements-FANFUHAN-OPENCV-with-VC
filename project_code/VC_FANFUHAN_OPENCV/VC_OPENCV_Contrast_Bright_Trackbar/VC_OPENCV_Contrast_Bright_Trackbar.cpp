// VC_OPENCV_Contrast_Bright_Trackbar.cpp : 定義主控台應用程式的進入點。
//OpenCV中軌跡條的建立 ~ https://www.itread01.com/content/1547088902.html [GOOGLE: opencv createTrackbar c++ ]
/*
// Debug | x32
通用屬性
| C/C++
...
Mat g_srcImage, g_dstImage;//輸入影象和輸出影象
static void on_ContrastAndBright(int, void*);//回撥函式

int main()
{

	g_srcImage = imread("../../images/test.png");
	if (!g_srcImage.data)
	{
	...
	{
		//定義輸入影象是和輸入影象同尺寸、同資料型別的零矩陣
		g_dstImage = Mat::zeros(g_srcImage.size(), g_srcImage.type());
		g_nContrastValue = 80;
		g_nBrightValue = 80;

		namedWindow("【效果視窗】");
		createTrackbar("對比度", "【效果視窗】", &g_nContrastValue,
			300, on_ContrastAndBright);
		createTrackbar("亮  度", "【效果視窗】", &g_nBrightValue,
